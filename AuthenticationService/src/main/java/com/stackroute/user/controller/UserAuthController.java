...
public class UserAuthController {
	...
	public ResponseEntity<?> postRegister(@RequestBody User user){
		try {
		boolean  postUser = userAuthService.saveUser(user);
	...
	*/
	...
	public ResponseEntity<?> login(@RequestBody User user){
		try {
			User cred= userAuthService.findByUserIdAndPassword(user.getUserId(), user.getPassword());
			if(cred == null) {
				return new ResponseEntity<String>("Invalid credentials",HttpStatus.UNAUTHORIZED);
			}
		} catch (UserNotFoundException e) {
			return new ResponseEntity<String>("UserNotFoundException",HttpStatus.UNAUTHORIZED);
		}
		String tok = getGenaratedToken(user.getUserId());
	...
	public String getGenaratedToken(String uname) {
		long expiry = 10_00_0000;
		return Jwts.builder().setSubject(uname).setIssuedAt(new Date(System.currentTimeMillis())).setExpiration(new Date(System.currentTimeMillis()+expiry))
